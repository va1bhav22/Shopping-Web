...
const b2cOrders = () => {
  ...
  const { data, mutate, isValidating } = useSWRAPI(`/orders?type=b2c`, {
  ...
  const b2cOrders = data?.data?.data?.data

  return (
    <AdminLayout title="Admin | Manage Orders ">
      <div className="m-auto px-4 py-4">
        ...
        <MaterialTable
          ...
          data={
            b2cOrders === null
              ? []
              : b2cOrders?.map((order: any, i: number) => ({
                  ...order,
                  sl: i + 1,
          ...
          columns={[
            ...
            },
            {
              ...
              field: 'totalPrice',
              render: (rowData) =>
                formatCurrency(rowData?.totalPrice ? rowData?.totalPrice : 0),
              editable: 'never',
              searchable: true,
              export: false,
            },
...
export default withAdmin(b2cOrders)
